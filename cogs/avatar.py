...
class Avatar(commands.Cog, name="avatar"):
    ...
    async def avatar(self, ctx, user_id=None):
        ...
        # getting user avatar
        file = await user.avatar.to_file(filename="avatar.png")

        circular_avatar = square_to_circle(file)
        circular_avatar = circular_avatar.resize((586, 586), Image.Resampling.LANCZOS)
