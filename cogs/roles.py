...
class Roles(commands.Cog, name="roles"):
    ...
    async def _panel_roles(self, context: Context) -> None:
        if context.interaction:
            ...
            await context.interaction.response.send_message("Создание панели через интеракцию...", ephemeral=True)
            self.bot.logger.info(type(context))
            ...
            await context.interaction.followup.send(embed=get_pick_rank_embed(), view=PickRankView(self.bot.database))
